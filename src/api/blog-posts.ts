...
export default async function handler(req: GatsbyFunctionRequest, res: GatsbyFunctionResponse) {
  ...
  try {
    ...
    if (!feedRes.ok) {
      return res.status(502).json({ error: "Could not fetch blog feed", articles: [] });
    }
    ...
    const parsed = (Array.isArray(entries) ? entries : [entries]).map((entry: any, i: number) => {
    ...
    return res.status(500).json({ error: "Internal server error", articles: [] });
  }
}
